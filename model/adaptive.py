...
class Caption(object):
    ...
    def __init__(self, sampled_ids, state, logprob, score, metadata=None):
...
class Encoder2Decoder( nn.Module ):
    ...
    # Caption generator
    def sampler_beam_search(self, images,beam_size, max_len=25):
        """
        ...
        _V=torch.cat([V for _ in range(beam_size)],0)
        _v_g=torch.cat([v_g for _ in range(beam_size)],0)
        #print("haha")

        _states = None

        initial_beam = Caption(
            sampled_ids=[Variable(torch.LongTensor(images.size(0), 1).fill_(1).cuda())],
            state=None,
            logprob=0.0,
            score=0.0,
            )
        ...
        for i in range(max_len):
            ...
            _captions = torch.cat([c.sampled_ids[-1] for c in partial_captions_list],0)
            #print(_captions.size())
            if i==0:
                _scores, _states, atten_weights, beta = self.decoder(V, v_g, _captions, _states)
                #print(_states)

            else:
                h_0_batch = torch.stack([c.state[0] for c in partial_captions_list],1)
                c_0_batch = torch.stack([c.state[1] for c in partial_captions_list],1)

                _captions=_captions.unsqueeze(1)

                _scores, _states, atten_weights, beta = self.decoder(_V, _v_g, _captions, (h_0_batch, c_0_batch))


            for j,partial_caption in enumerate(partial_captions_list):
                softmax = F.softmax(_scores[j])
                ...
                result=torch.topk(softmax, beam_size, 1)
                ...
                for index in range(beam_size):
                    ...
                    p=result[0][0][index]
                    #print(p)
                    if (p<1e-12).cpu().data.numpy():
                        continue
                    sentence = partial_caption.sampled_ids + [w]
                    logprob = partial_caption.logprob + np.log(p.cpu().data.numpy())
                    score = logprob
                    if (w==2).cpu().data.numpy():
                        beam = Caption(sentence,(h_0,c_0), logprob, score)
        ...
        temp=complete_captions.extract(sort=True)
        return temp[0].sampled_ids
